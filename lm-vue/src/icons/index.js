 ...
 */
import Vue from 'vue'
import pageIconSvg from '@/components/page-components/page-icon-svg/page-icon-svg.vue' 
Vue.component('pageIconSvg', pageIconSvg)
const svgFiles = require.context('@/icons/svg', true, /\.svg$/);
let iconLists = [], eleIcons = [];
if (typeof window !== 'undefined' && typeof window.navigator !== 'undefined') {
  iconLists = svgFiles.keys().map(item => svgFiles(item));
}
export default {
  // 获取图标icon-(*).svg名称列表, 例如[shouye, xitong, zhedie, ...]
  getNameList () {
    return eleIcons.concat(iconLists.map(item => item.default.id.split('-')[1]));
  }
}
