<template>
  <div class="vup-popup-header" :class="showBottomBorder ? 'vup-border-bottom' : ''">
    <div class="vup-popup-header-left" @click="$emit('on-click-left')">
      <slot name="left-text">{{ leftText }}</slot>
    </div>
    <div class="vup-popup-header-title">
      <slot name="title">{{ title }}</slot>
    </div>
    <div class="vup-popup-header-right" @click="$emit('on-click-right')">
...
.#{$class-prefix} {
  ...
  &-popup-header.#{$class-prefix}-1px-b:after {
    border-color: #D9D9D9;
  }
}
</style>